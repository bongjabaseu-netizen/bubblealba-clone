...
export function PostsClient({ posts }: { posts: Post[] }) {
  ...
  const boardNames = useMemo(() => {
    ...
    posts.forEach((p) => { if (p.board?.name) names.add(p.board.name); });
  ...
  function handleDelete(postId: string) {
    ...
    startTransition(async () => { await deleteAdminPost(postId); router.refresh(); });
  }
  function handleTogglePin(postId: string) {
    startTransition(async () => { await adminTogglePinPost(postId); router.refresh(); });
  }
  ...
  const notices = filtered.filter((p) => p.pinned);
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-[13px] text-mute">
          ...
          커뮤니티 최상단 고정 공지·이벤트 <b className="text-slate-300">{notices.length}건</b>
        </div>
      ...
      <div className="flex items-center gap-3">
        ...
        {(search || boardFilter !== "ALL") && <span className="text-[13px] text-mute">{filtered.length}건</span>}
      </div>
      ...
      <div className="rounded-2xl bg-navy-850/90 border border-line shadow-card overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-line">
          ...
          <span className="text-[12px] text-mute">총 {posts.length}개 · 고정 {posts.filter((p) => p.pinned).length}</span>
        </div>
        <table className="w-full text-[12.5px]">
          ...
          <tbody className="divide-y divide-line/50 text-slate-300">
            {filtered.length === 0 ? (
              <tr><td colSpan={7} className="text-center py-12 text-mute">게시물이 없습니다</td></tr>
            ) : (
              filtered.map((p) => (
      ...
      {editPost && <EditPostModal post={editPost} onClose={() => setEditPost(null)} onSaved={() => { setEditPost(null); router.refresh(); }} />}
    </div>
  );
}
