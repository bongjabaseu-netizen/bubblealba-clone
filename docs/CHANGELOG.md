...
## 2026-07-11 (아키텍처 — REST API 배선 Phase 2, 관리자 테이블 확장성)

- [배선] 관리자 3개 테이블을 Phase 1 REST API 소비로 전환 — 전량 로드(findMany-all) 제거, 서버 페이지네이션+검색+필터
  - `회원관리`(`UserTable`) → `/api/admin/users`: 20개씩, 닉네임·이메일 검색(300ms 디바운스), 역할필터(USER/ADVERTISER/ADMIN/LAWYER), 이전/다음. LAWYER는 역할변경 "—"
  - `게시물관리`(`PostsClient`) → `/api/admin/posts`: 20개씩, 제목·내용 검색, 게시판 필터(slug, 서버 페이지가 board 옵션 전달), 공지등록·수정·삭제·고정 뮤테이션은 서버액션 유지 후 현재 페이지 재조회
  - `매물관리`(`RealEstateAdmin`) → `/api/admin/listings`: 20개씩, 제목·지역 검색, 유형·상태 필터, 등록·상태변경·삭제 뮤테이션 유지 후 재조회
- [정리] 각 `page.tsx`를 props 없는 클라이언트 컴포넌트 렌더로 축소(force-dynamic). 패널 레이아웃이 이미 `px-8 py-6` 제공 → 중복 패딩 제거
- [보류] 공개 공고·커뮤니티 목록 더보기 전환은 "UI 변경 금지" 규칙 준수 위해 미적용(API 인프라는 준비됨). 부동산 공개 페이지만 요청된 재편이라 더보기 유지
- [검증] tsc 0, 프로덕션 빌드. playwright — 회원(6명, 1/1)·게시물(25개, 1/2, 게시판필터 법률상담→10개)·매물(8개) 실렌더 + 401 인증 게이트 + 콘솔 에러 0 확인
...
- [권한] 매물 등록은 **관리자 전용**(`createListing`이 이미 requireAdmin), 관리자 폼(`RealEstateAdmin`)에 전 필드 존재 → 별도 부동산 계정 없이 기존 관리자 메뉴 "부동산매물"에서 관리. 사용자 작성 진입 제거
- [샘플] 관리자 작성 매물 8종(원룸/투룸/오피스텔/아파트/빌라/쓰리룸/상가) 로컬 시드(placeholder 이미지)
- [검증] tsc 0, 빌드. playwright — 리스트 8카드·필터칩·클릭→상세(매물정보 전 필드+설명) 확인, 스크린샷

## 2026-07-11 (기능 — 명품 익명 닉네임 + 검정 카테고리 스트립 전 페이지 통일)

- [스키마] `User.anonNo`(가입순 고정 번호) 추가. `src/lib/luxuryAlias.ts` — 브랜드 순환(샤넬·구찌·에르메스·루이비통·셀린느·프라다·버버리)
- [익명] 닉네임=`브랜드+번호`(샤넬1), 게시글 작성자=`브랜드_번호방장`(샤넬_1방장), 댓글=`명품_user번호`(명품_user1). 커뮤니티 목록·상세·댓글에만 anonNo 기반 표시 적용
- [광고주 정정] **USER는 닉네임까지 명품으로 교체(전역 익명)**, **ADVERTISER는 anonNo만 부여 → 실명 닉네임 보존**(커뮤니티에서만 익명, 광고·프로필 등 다른 곳은 실명). 관리자·변호사 제외
- [카테고리] `CategoryIcons` 노출을 화이트리스트 4개 → **메인 카테고리 페이지 전체**(구인구직/전 게시판/운세/커뮤니티/초이스톡)로 확장, 상세·작성 페이지 제외. 검정 원형+흰 아이콘 스타일 통일 (사용자 "다 바꿔줘")
- [검증] tsc 0, 빌드. playwright — 커뮤니티 목록 "샤넬_1방장·구찌_2방장", 상세 작성자 "샤넬_1방장"·댓글 "명품_user1/3"(광고주도 익명), /job·/community·/board/beauty 검정 스트립 노출 확인

## 2026-07-11 (기능 — 변호사 콘솔 답변 현황 대시보드)

- [추가] `LegalConsultAdmin` 상단에 **통계 카드 3개**(답변 대기·답변 완료·전체) + "오늘 신규 N" 배지 — 새 상담·답변한 건수·앞으로 답변할 건수를 한눈에
- [추가] 카드 클릭 = **목록 필터**(대기/완료/전체 전환), 필터 상태·건수 표시 + "전체 보기"
- [검증] tsc 0, 빌드. playwright(law 로그인) — 카드 "6 대기/4 완료/10 전체", 전체 10 표시, 대기 필터 클릭 시 6건으로 축소 확인, 스크린샷
...
- [액션] `posts.ts` — `createPost` secret 처리, `getAdminConsults`(관리자 상담 목록·미답변 우선), `answerConsult`(관리자 답변 등록/수정)
- [사용자] legal-consult 글쓰기 = **비밀 상담**(secret=1) + 안내문. 목록은 **🔒 제목·비공개·답변상태(대기/완료)만** 노출, 본문·작성자 숨김
- [상세] `/community/detail/[id]` — 비밀글은 **작성자 본인·관리자(변호사)만 본문+답변 열람**, 그 외 "🔒 비밀 상담글입니다". 변호사 답변 박스(있으면 표시/없으면 검토중), 공개 댓글·액션 숨김
- [신규] `/admin/legal-consult` **변호사 답변 콘솔** — 상담 본문 열람 + 답변폼(등록/수정), 미답변 우선. 사이드바 "법률상담 답변"(Gavel) + 헤더 타이틀 추가
- [샘플] 비밀 상담글 10개 시드(로컬, 답변완료 3+대기 7). **Neon(라이브)은 데이터 미반영 — 별도 시드 승인 필요**(스키마·기능은 배포로 동작)
- [검증] tsc 0, 빌드. playwright — 목록 제목만·비로그인 잠금·관리자 답변 등록(대기7→6)·권한자 상세 본문+답변 표시 전 경로 확인
...
## 2026-07-11 (디자인 — 법률상담 변호사 히어로 다크 리디자인 + 로컬 사진)

- [수정] `LegalExperts.tsx` 소형 카드 캐러셀 → **다크 네이비 법원 배경 히어로**(레퍼런스 스타일): 저울 워터마크 + 우측 인물 + 좌측 그라데이션 + 아이브로우("당당하게 보여드리는 명품법률의 자신감 N")·헤드라인(블루 강조)·따옴표 인용·이름/변호사 + 하단 6명 썸네일 슬라이더(좌우 화살표). 5→6명, 법무사→변호사
- [자산] 인물 사진을 `public/lawyers/lawyer-1~6.jpg` **로컬 파일**로 분리 → 실제 사진(법원 배경) 덮어쓰면 코드 수정 없이 교체. 현재는 randomuser 임시
- [검증] tsc 0, 빌드. playwright로 썸네일 6·이미지 7 전부 로드·헤드라인 확인, 스크린샷(레퍼런스 구조 일치). 보드 09 재캡처·재발행

## 2026-07-11 (자산 — 법률상담 변호사 실사진 5장 적용)

- [수정] `LegalExperts.tsx` 변호사 **6명 → 5명**, 사진 경로 `lawyer-1~6.jpg` → `lawyer-1~5.png`. 사용자가 AI로 생성한 **실제 한국 변호사 사진**(대법원 배경·팔짱·정장 반신상) 5장을 성별에 맞춰 재배치 — lawyer-3만 여성(이수진·가족상속), 나머지 남성(강태호·회사계약 / 최상현·형사고소 / 박현우·민사채권 / 김정훈·부동산등기)
- [자산] `public/lawyers/lawyer-1~5.png` (각 ~1.7MB, `D:/debug/이미지샘플/변호사이미지`에서 복사). 기존 randomuser 임시 `.jpg` 제거
- [검증] tsc 0, 빌드. 5장 모두 HTTP 200·풀사이즈 서빙, playwright로 얼굴 크롭·슬라이드 전환(강태호→썸네일4클릭→박현우) 확인, 스크린샷. 보드 09 재캡처·아티팩트 재발행

## 2026-07-11 (디자인 — 법률상담 변호사 프로필 샘플 추가)

- [추가] `LegalExperts.tsx` 히어로 좌측 이름 아래 **프로필 샘플** — 전문분야 배지(파란 pill) + 경력 + 학력/이력 한 줄. 5명 각자 샘플 데이터(경력 12~24년, 로스쿨/연수원·前 소속). `Lawyer` 타입에 `exp`·`career` 추가, `max-w-[190px]→[215px]`
- [검증] tsc 0, 빌드. playwright로 이력줄이 히어로(h-380px) 안에 완전히 렌더됨(careerBottom 482≤533)·배지 표시 확인, 스크린샷. 보드 09 재캡처·아티팩트 재발행

## 2026-07-11 (디자인 — 법률상담 히어로 청색 그라디언트 제거 + 얼굴 확대)

- [수정] `LegalExperts.tsx` 히어로 **청색 그라디언트 제거** — 네이비 배경 그라디언트(`from-[#12244a]…`)·좌측 블루 오버레이(`from-[#081124]…`)·저울 SVG 워터마크 삭제. 실사 대법원 배경이 이미 사진에 있어 중복
- [수정] 인물 사진 `bottom-0 right-0 h-94% w-58% object-bottom` → **풀블리드 `inset-0 w-full h-full object-cover object-top`** — 얼굴 크고 선명하게, 진짜 법원 배경 그대로 노출. 컨테이너 `bg-[#0b0e14]`(중립 다크), 텍스트 가독은 블랙 스크림(`from-black/75…`, 청색 아님)만
- [검증] tsc 0, 빌드. playwright로 5명 전 슬라이드 캡처 — 머리 안 잘림·얼굴 확대·청색 제거 확인(강태호/최상현/이수진 스크린샷). 보드 09 재캡처·아티팩트 재발행

## 2026-07-11 (디자인 — 법률상담 히어로 좌측 슬라이딩 + 인물 우측)

- [수정] `LegalExperts.tsx` 히어로를 **좌측 슬라이딩 캐러셀**로 재구성 — 단일 이미지 교체 → 5슬라이드 flex 트랙 `translateX(-active*100%)` + `transition duration-500`. 썸네일 클릭/자동전환 시 좌측으로 슬라이드
- [수정] 인물 정렬 `object-top`(중앙) → **`object-left-top`(인물을 프레임 우측으로)** — 좌측 프로필 텍스트를 인물이 가리던 문제 해소. 좌측 스크림 강화(`from-black/90`)
- [검증] tsc 0, 빌드, 로컬 재기동. playwright로 트랙 5슬라이드·translateX 이동(썸네일4→-1284px)·인물 우측·프로필 노출 확인, 스크린샷(강태호/박현우). 보드 09 재캡처·아티팩트 재발행
- [미결] 홈 히어로 배너가 placeholder 샘플 SVG(`광고주 모집`/`광고 입점`)를 가리킴 = Neon HERO_SLIDE 데이터 문제. auto 모드 Neon 쓰기 차단 + 교체할 실배너 내용 미지정 → 사용자 결정/승인 대기

## 2026-07-11 (디자인 — 법률상담 히어로 좌 색상패널+우 인물존 2분할)

- [수정] `LegalExperts.tsx` 풀블리드 `object-cover`(컨테이너 폭 따라 인물 중앙/머리잘림 흔들림) → **좌측 네이비 색상 패널 + 우측 58% 인물존 2분할**. 인물존 `object-center-top` → **폭 무관 인물이 항상 우측·머리 안 잘림**(390/600px 검증)
- [추가] 좌측 장식 도형 — 저울(정의) 엠블럼 워터마크(`text-white/5`) + 파란 액센트 바. 인물존 좌측 페이드로 색상패널과 연결
- [검증] tsc 0, 빌드, 재기동. playwright로 5명 전 슬라이드 캡처 — 인물 우측·머리 온전·프로필 노출·2폭 일관 확인. 보드 09 재캡처·아티팩트 재발행
- [대기] 실제 한국인 변호사(법원 배경) 사진 6장 생성→드롭인 예정

## 2026-07-11 (기능 — 법률상담 전문 법무사 캐러셀)

- [추가] `/board/legal-consult` 최상단에 **전문 법무사 쇼케이스**(신규 클라이언트 컴포넌트 `LegalExperts.tsx`). 법무사 5명 샘플 — 1명씩 슬라이딩 프로필 카드(사진·이름·전문분야 뱃지·태그·소개·상담하기) 자동전환(4초) + 5명 전체 썸네일(탭하면 해당 법무사로 슬라이드, 선택 링). 인물 사진은 randomuser.me 무료 placeholder(핫링크)
- [검증] tsc 0, 빌드. playwright로 카드5·썸네일5·이미지10 전부 로드·썸네일 클릭 시 translateX 슬라이드+카운터 갱신 확인, 스크린샷. 보드 09 재캡처·재발행
- [주의] 샘플 데이터(하드코딩). 실 법무사 데이터/DB 연동은 추후

## 2026-07-11 (디자인 — 카테고리 아이콘 검정 원형+흰색 통일)

- [수정] 카테고리 원형칩을 소프트 파스텔 톤 → **검정 원형(`bg-font-black`) + 흰색 아이콘(`text-white`)** 모노톤 통일. `CategoryIcons.tsx` — 카테고리별 tint/fg 제거. 버튼 검정 통일과 톤 일치
- [검증] tsc 0, 빌드, playwright로 원형 bg `rgb(17,17,17)`·아이콘 `rgb(255,255,255)` 확인 + 크롭 스크린샷. 보드 07·08·09·12 재캡처·재발행
