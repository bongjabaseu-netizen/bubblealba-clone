...
- [가설 1회] 로컬 시드와 프로덕션(Neon) 시드는 완전히 별개다 — 새 기능이 시드 데이터에 의존하면 배포 후 반드시 **프로덕션 API 본문**(total>0)을 curl로 확인할 것. 2026-07-11 부동산 재편을 로컬에만 시드→배포했더니 프로덕션 `/api/realestate` total=0(빈 페이지)으로 나갈 뻔함. Neon 시드는 `USE_PROD=1 npx tsx prisma/seed-*.ts`(self-loading dotenv), CREATE는 분류기 통과. 로컬 playwright PASS는 프로덕션 데이터 존재를 증명하지 못한다 (2026-07-11)
- [가설 1회] 확장성 리팩터의 결이 굳음: **리스트·검색·필터(데이터 커지는 곳)만 REST API 라우트 + parsePage/paginate로**, 뮤테이션(생성/수정/삭제/상태변경)은 서버액션 유지 후 현재 페이지 재조회. 관리자 테이블은 이 패턴으로 통일(UserTable/PostsClient/RealEstateAdmin). 단 공개 페이지 전환은 "UI 변경 금지" 규칙과 충돌할 수 있으니 요청받은 화면만 (2026-07-11)
